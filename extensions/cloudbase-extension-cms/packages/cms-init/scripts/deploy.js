...
const { ActivitySchema, TaskSchema, MySchemas } = require('./_schema')
...
module.exports = {
  ...
  async deploySchema(context) {
    const project = {
      "_id": "18ed09686177790201ec19ef3227319f",
      "customId": "miniprogram",
      "name": "小程序"
    };
    await addProject(project, context)

    const { isMpEnv } = context
    for (let i = 0; i < MySchemas.length; i++) {
      await addSchema(MySchemas[i], context);
    }
    // 仅微信环境下创建活动 schema
    if (!isMpEnv) {
      return
...
// 添加模板 project
async function addProject(project, context) {
  // 添加 project
